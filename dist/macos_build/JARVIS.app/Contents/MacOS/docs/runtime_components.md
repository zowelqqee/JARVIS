...
### Answer Engine
The Answer Engine builds grounded read-only answers. It may use docs, structured capability metadata, current runtime visibility, and session context. The default v1 path is deterministic. An opt-in LLM-backed backend may exist behind the same `Answer Engine` contract, but routing, grounding, and safety policy must remain outside the backend. Backend selection should come from explicit QA configuration, grounding should be assembled into a source bundle before generation, shared source-attribution and groundedness verification should live in a backend-agnostic verifier module, provider-specific request construction should stay behind the backend seam, and HTTP transport should stay behind a dedicated provider transport adapter. If a configured LLM backend is unavailable or returns an ungrounded answer, the Answer Engine should either fall back to the deterministic backend with a visible warning or fail honestly. The model-backed answer payload must stay versioned; the current frozen schema version is `qa_answer_v1`.
