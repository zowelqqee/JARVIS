...
- The same explicit-choice requirement is enforced when reading old artifacts: `qa beta` now rejects a recorded ready artifact unless it explicitly says that the chosen candidate came from operator selection rather than implicit recommendation drift.
- The same rule now applies one layer earlier too: `qa beta` rejects a stored beta release-review artifact unless it explicitly records operator-selected candidate choice, and `python3 -m qa.beta_release_review --write-artifact` requires `--candidate-profile` instead of inheriting the candidate silently.
...
- Release-review consistency is freshness-aware too: even a fresh `tmp/qa/beta_release_review.json` must be discarded if the latest `tmp/qa/manual_beta_checklist.json` has already gone stale, because the sign-off is no longer tied to current manual evidence.
