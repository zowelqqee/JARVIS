...
from interaction_result import InteractionMode, InteractionResult  # type: ignore  # noqa: E402
...
class InteractionManager:
    ...
    answer_backend_kind: AnswerBackendKind = AnswerBackendKind.DETERMINISTIC
    ...
    def handle_input(self, raw_input: str, session_context: SessionContext | None = None) -> InteractionResult:
        ...
        if decision.kind == RoutedInteraction.CLARIFICATION:
            return InteractionResult(
                interaction_mode=InteractionMode.CLARIFICATION,
        ...
        if decision.kind == RoutedInteraction.COMMAND:
        ...
        try:
            answer_result = answer_question(
                ...
                backend_kind=self.answer_backend_kind,
            )
        except InputNormalizationError as exc:
            return InteractionResult(
                interaction_mode=InteractionMode.QUESTION,
                normalized_input=decision.normalized_input,
                error=self._input_error(exc),
            )
