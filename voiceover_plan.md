...
## 27. Актуальный статус на 31 марта 2026

На текущий момент voice layer уже заметно дальше исходного MVP.

### 27.1 Что уже собрано

- [x] `voice/tts_provider.py` и `voice/tts_macos.py`
- [x] отдельный `voice/speech_presenter.py`
- [x] speech-specific rendering для:
  - question answers;
  - command completions;
  - clarification prompts;
  - confirmation prompts;
  - warnings;
  - failures.
- [x] базовый `voice/session.py` и `voice/dispatcher.py`
- [x] half-duplex `voice/audio_policy.py`
- [x] bounded multi-turn voice loop под feature flag
- [x] voice telemetry, readiness helper и rollout gate
- [x] voice eval corpus для русского и английского path
- [x] manual verification runbook
- [x] speech hardening против:
  - длинных путей;
  - markdown/evidence noise;
  - policy-heavy refusals;
  - debug tails.

### 27.2 Что ещё не дотягивает до "идеала"

- [ ] нет подтверждённого live microphone smoke в текущем цикле работ
- [ ] advanced conversation mode всё ещё rollout-only и не validated как default UX
- [ ] нет полноценного barge-in и mid-utterance TTS interruption
- [ ] нет earcons / spoken latency fillers
- [ ] conversation loop остаётся bounded и half-duplex, а не truly fluid
- [ ] русский deterministic command surface ещё можно расширять
- [ ] нет финального критерия, когда voice mode можно включать шире без флага

## 28. План доведения до идеала

Ниже описан уже не MVP-план, а practical path от текущего состояния к voice mode, который можно считать почти идеальным для текущей архитектуры JARVIS.

### 28.1 Milestone A. Live-ready voice beta

Цель:

- подтвердить, что текущая архитектура работает не только в unit/eval, но и в реальном микрофонном сценарии.

Подзадачи:

- [ ] Пройти полный `docs/manual_voice_verification.md` вручную на macOS.
- [ ] Проверить отдельно:
  - русский ASR;
  - английский ASR;
  ...
  - speech output при `speak on`.
- [ ] Записать:
  - `voice telemetry write`;
  - `voice readiness write`;
  - `voice gate`.
- [ ] Исправить только реальные live regressions, найденные в этом smoke.
- [ ] Не включать advanced mode по умолчанию до завершения этого milestone.

Acceptance criteria:

- live checklist проходит без blocker'ов;
- readiness artifact собран;
- `voice gate` показывает `ready` для текущего bounded voice mode.

### 28.2 Milestone B. Conversation UX без ощущения "терминала с микрофоном"

Цель:

- сделать bounded conversation mode заметно более естественным, не ломая text-first core.

Подзадачи:

- [ ] Уточнить политику auto re-listen:
  - когда повторно слушать после clarification;
  - когда повторно слушать после confirmation;
  - когда не открывать follow-up после длинного ответа.
- [ ] Довести voice control surface:
  - `listen again`;
  - `stop speaking`;
  - `repeat`;
  - `say more`;
  - русские эквиваленты.
- [ ] Поддержать чуть более умное закрытие follow-up window:
  - explicit dismiss;
  - timeout;
  - no-speech fallback;
  - bounded retry budget.
- [ ] Добавить короткие spoken latency fillers для долгого answer generation.
- [ ] Добавить опциональные earcons:
  - listening start;
  - listening stop;
  - error;
  - speaking start.
...
- 2-3 голосовых turn подряд ощущаются как связный разговор;
- follow-up окно не выглядит случайным или навязчивым;
- пользователю не нужно постоянно возвращаться к клавиатуре.

### 28.3 Milestone C. Audio excellence

Цель:

- закрыть основные UX-дырки, которые отделяют хороший half-duplex assistant от почти "магического" voice mode.

Подзадачи:

- [ ] Реализовать controlled TTS interruption.
- [ ] Явно определить barge-in contract:
  - что считается прерыванием;
  - когда TTS прекращается;
  - когда начинается новая capture session.
- [ ] Доработать `voice/audio_policy.py` для:
  - suppression of self-feedback;
  - stop-speaking-before-listening;
  - pause/resume semantics.
- [ ] Подготовить future hook для VAD / silence tuning.
- [ ] Зафиксировать UX на noisy/missed follow-up turns.

Acceptance criteria:

- TTS не мешает следующему capture;
- user может прерывать длинный spoken answer контролируемо;
- нет заметных feedback loops между `say` и capture.

### 28.4 Milestone D. Language and content quality

Цель:

- довести spoken output до уровня, где он consistently звучит как ассистент, а не как адаптированный terminal log.

Подзадачи:

- [ ] Довести language-aware TTS choice до явной policy:
  - user request locale;
  - spoken message language;
  - explicit override.
- [ ] Добавить более умное summarization для длинных open-domain answers.
- [ ] Отдельно формировать spoken source references:
  - короткие file labels;
  - short host names;
  - no raw evidence dumps.
- [ ] Финализировать refusal/warning templates:
  - краткие;
  - consistent;
  - без policy wall-of-text.
- [ ] Расширить русские deterministic spoken templates там, где ещё слышен English fallback.

Acceptance criteria:

- spoken output почти никогда не звучит как screen log;
- длинные ответы остаются полезными на слух;
- ru/en voice surface выглядит целостным.

### 28.5 Milestone E. Release-grade rollout

Цель:

- сделать voice mode не только хорошим, но и безопасным для поэтапного включения.

Подзадачи:

- [ ] Сделать nightly/regular voice eval прогон частью release hygiene.
- [ ] Стабилизировать readiness artifacts и telemetry artifacts как operator workflow.
- [ ] Ввести явный критерий default-on readiness для advanced voice mode.
- [ ] Разделить rollout уровни:
  - single-turn default;
  - bounded conversation opt-in;
  - future advanced conversation.
- [ ] Зафиксировать rollback strategy:
  - feature flag off;
  - TTS off;
...
- rollout можно делать по уровням риска;
- regressions замечаются быстро;
- advanced mode можно включать осознанно, а не "на удачу".

## 29. Идеальное целевое состояние

Voice mode можно считать близким к идеалу для этой codebase, когда одновременно выполняется следующее:

- JARVIS стабильно понимает короткие русские и английские utterances без ручного rephrase.
- Question/command/clarification/confirmation path работают одинаково надёжно и с клавиатуры, и голосом.
- Spoken output короткий, языково-consistent и не читает логи, пути, markdown или debug noise.
- После clarification и confirmation разговор естественно продолжается голосом.
- Длинный ответ можно:
  - прервать;
  - повторить;
  - попросить сократить;
  - уточнить по источникам.
- `voice telemetry`, `voice readiness` и `voice gate` дают оператору реальную картину качества.
- Advanced conversation mode можно включать не потому, что "кажется готово", а потому что есть evidence.

## 30. Рекомендуемый актуальный порядок работ

Если двигаться от состояния репозитория на 31 марта 2026, лучший порядок теперь такой:

1. Закрыть live manual verification и собрать artifacts readiness/telemetry.
2. Исправить только реальные проблемы, найденные в live voice smoke.
3. Доработать conversation UX:
   - retries;
   - timeouts;
   - fillers;
   - earcons.
4. После этого идти в controlled barge-in и TTS interruption.
5. Затем финализировать language/content quality для spoken path.
6. Только после этого принимать решение, можно ли расширять default rollout voice mode.

Это даёт максимальный product gain без лишнего архитектурного рефакторинга и сохраняет text-first core как главный инвариант.
